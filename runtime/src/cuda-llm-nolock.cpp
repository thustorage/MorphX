    ...
    CUstream hStream, void** kernelParams, void** extra) 
{
    ...
    // if (1
    //     && strstr(funcName, "NSL_IfLi4ELb1EEESF_EENS1R_25Til") == nullptr
    //     && strstr(funcName, "redLoadIteratorMixedINS1S_1") == nullptr
    //     && strstr(funcName, "BatchDecode") == nullptr
    // ) {
    ...
    if ((gWaveTime.count(funcName) && gWaveTime[funcName] < 50.0)) {
        return real_cuLaunchKernel(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes, hStream, kernelParams, extra);
    }
    ...
    // if(gCI.count(funcName) && gCI[funcName] >= 0.99) {
    //     launchMutex.unlock();
    //     return CUDA_SUCCESS;
    // }
