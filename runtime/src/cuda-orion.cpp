...
void generateProfileResult(KernelInfo *info, std::vector<int> &profileWaves, std::vector<NanoSec> &profileTimes) {
    ...
    if(CI >= 0.75) CI = 1;
    gCI[info->name] = CI;
    printf("[smsched] kernel %s: occup: %d, CI: %lf, WaveTime: %lfus\n", info->name, info->occup, CI, tm / 1000.0);
    ...
    CUstream hStream, void** kernelParams, void** extra) 
{
    ...
    int priority = 0;
    real_cuStreamGetPriority(hStream, &priority);
    if(priority < 0) {
    ...
    if(be_event != nullptr) {
        real_cuStreamWaitEvent(hStream, be_event, 0);
    }
    auto res = real_cuLaunchKernel(f, gridDimX, gridDimY, gridDimZ, 
        blockDimX, blockDimY, blockDimZ, sharedMemBytes, 
        hStream, kernelParams, extra);
    real_cuEventRecord(be_event, hStream);
    return res;
}
