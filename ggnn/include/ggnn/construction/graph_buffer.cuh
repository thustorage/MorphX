...
struct GraphBuffer {
  ...
  /// combined buffer for temporary storage
  Dataset<std::byte> memory{};

  /// distance to nearest known neighbor per point
  /// (generated by merge kernel, used by selection kernel)
  ValueT* nn1_dist_buffer{nullptr};
  /// random numbers
  /// (generated by cuRAND, used by selection kernel)
  float* rng{nullptr};
  // cub buffer for computing mean and max over nn1 dists
  std::byte* temp_storage_cub{nullptr};
  size_t temp_storage_bytes_cub{0};

  /// temporary storage of new edges computed by merge kernel
  /// copied into the graph, once merge kernel is finished
  KeyT* graph_buffer{nullptr};

  /// sym edges to be inserted per point
  KeyT* sym_buffer{nullptr};
  /// number of sym edges to be inserted per point
