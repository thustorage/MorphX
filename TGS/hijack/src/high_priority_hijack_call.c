...
static int open_clientfd(CUdevice device) {
  ...
  */
  ...
  sprintf(SOCKET_PATH, "/home/rtx/gpu/sm-sched/TGS/gsharing/rate_%s.sock", uuid_str);
  ...
  printf("High priority connected to rate watcher socket: %s\n", SOCKET_PATH);
  fflush(stdout);
  return clientfd;
}
...
CUresult cuMemAlloc_v2(CUdeviceptr *dptr, size_t bytesize) {
  // printf("cuMemAlloc_v2 called: bytesize(%zu)\n", bytesize);
  // fflush(stdout);
  CUresult ret;
  ret = CUDA_ENTRY_CALL(cuda_library_entry, cuMemAllocManaged, dptr, bytesize,
                        1);
...
CUresult cuMemAlloc(CUdeviceptr *dptr, size_t bytesize) {
  // printf("cuMemAlloc called: bytesize(%zu)\n", bytesize);
  // fflush(stdout);
  CUresult ret;
...
CUresult cuLaunchKernel(CUfunction f, unsigned int gridDimX,
  ...
  rate_estimator(gridDimX * gridDimY * gridDimZ);
  // printf("cuLaunchKernel called: gridDim(%u,%u,%u)\n", gridDimX, gridDimY, gridDimZ);
  // fflush(stdout);
...
CUresult cuModuleLoadData(CUmodule* module, const void* image) {
    ...
    CUresult result = CUDA_ENTRY_CALL(cuda_library_entry, cuModuleLoadData, module, image);
    return result;
}
...
CUresult cuLibraryLoadData(CUlibrary* library, const void* code, CUjit_option* jitOptions, void** jitOptionsValues, unsigned int numJitOptions, CUlibraryOption* libraryOptions, void** libraryOptionValues, unsigned int numLibraryOptions) {
    ...
    CUresult result = CUDA_ENTRY_CALL(cuda_library_entry, cuLibraryLoadData, library, code, jitOptions, jitOptionsValues, numJitOptions, libraryOptions, libraryOptionValues, numLibraryOptions);
    return result;
}

CUresult cuModuleLoadDataEx(CUmodule* module, const void* image, unsigned int numOptions, CUjit_option* options, void** optionValues) {
  fprintf(stderr, "[mod] cuModuleLoadDataEx called for image %p\n", image);
  CUresult ret = CUDA_ENTRY_CALL(cuda_library_entry, cuModuleLoadDataEx, module, image, numOptions, options, optionValues);
  return ret;
}
...
CUresult cuModuleLoadFatBinary(CUmodule* module, const void* fatCubin) {
  ...
  CUresult result = CUDA_ENTRY_CALL(cuda_library_entry, cuModuleLoadFatBinary, module, fatCubin);
  return result;
}
...
CUresult cuModuleGetFunction(CUfunction* hfunc, CUmodule hmod, const char* name) {
    ...
    CUresult result = CUDA_ENTRY_CALL(cuda_library_entry, cuModuleGetFunction, hfunc, hmod, name);
    return result;
}

CUresult cuKernelGetFunction(CUfunction* pFunc, CUkernel kernel) {
    fprintf(stderr, "[mod] cuKernelGetFunction called for %p\n", kernel);
  CUresult result = CUDA_ENTRY_CALL(cuda_library_entry, cuKernelGetFunction, pFunc, kernel);
    return result;
}

CUresult cuLibraryGetKernel(CUkernel* pKernel, CUlibrary library, const char* name) {
    fprintf(stderr, "[mod] cuLibraryGetKernel called for %s\n", name);
  CUresult result = CUDA_ENTRY_CALL(cuda_library_entry, cuLibraryGetKernel, pKernel, library, name);
    return result;
}

CUresult cuLibraryGetModule(CUmodule* pMod, CUlibrary library) {
    fprintf(stderr, "[mod] cuLibraryGetModule called for library %p\n", library);
  CUresult result = CUDA_ENTRY_CALL(cuda_library_entry, cuLibraryGetModule, pMod, library);
    return result;
}
...
CUresult cuGetProcAddress_v2(const char* symbol, void** pfn, int cudaVersion, cuuint64_t flags, CUdriverProcAddressQueryResult* symbolStatus) {
  ...
  CUresult result = CUDA_ENTRY_CALL(cuda_library_entry, cuGetProcAddress_v2, symbol, pfn, cudaVersion, flags, symbolStatus);
  return result;
}
...
CUresult cuGetProcAddress(const char* symbol, void** pfn, int  cudaVersion, cuuint64_t flags, CUdriverProcAddressQueryResult* symbolStatus) {
  ...
  CUresult result = CUDA_ENTRY_CALL(cuda_library_entry, cuGetProcAddress, symbol, pfn, cudaVersion, flags, symbolStatus);
  return result;
}